...
namespace UnitTests
{
	public class UT_FinanceLogic : IClassFixture<UT_FinanceLogic_Fixture>
	{
		...
		public void GetAllAccounts(bool openOnly)
		{
			...
			// ACT
			List<Account> actAccts = FinanceLogic.GetAllAccounts(openOnly);

			// ASSERT
			Assert.Equal(3, actAccts.Count);
		}
	}

	public class UT_FinanceLogic_Fixture
	{
		private string StaticDbFilename;
		public UT_FinanceLogic_Fixture()
		{
			...
			{
				using (var context = new FinanceModelContext(@"DataSource=" + StaticDbFilename))
				{
